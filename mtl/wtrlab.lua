...
local PREF_PROXY_URL    = "wtrlab_proxy_url"     -- string, default: https://wtr-lab-proxy.fly.dev/chapter
local PREF_SERVICE_DELIVERY = "wtrlab_service_delivery" -- "json" | "query"
...
local function getServiceDelivery()
    ...
    return "json" -- default: send service in JSON body
end
...
local function http_get_retry(url, config, retries)
    ...
    for attempt = 1, retries do
        ...
        if attempt < retries then
            sleep(200 * attempt) -- backoff: 200ms, 400ms, 600ms
        end
    end
...
function getChapterText(html, chapterUrl)
    ...
    if mode == "raw" then
        ...
        serviceParam = nil        -- no service, just raw web version
    elseif mode == "web" then
        translateParam = "web"
        languageParam = "en"
        serviceParam = "web"      -- request Google translation
    elseif mode == "webplus" then
        translateParam = "webplus"
        languageParam = "en"
        serviceParam = "webplus"  -- Google translation + glossary (server‑side)
    elseif mode == "ai" then
        translateParam = "ai"
        languageParam = "none"
        serviceParam = nil
    ...
    -- Function to perform the API request (with optional service param)
    local function doRequest(withServiceInJson)
        local bodyObj = {
        ...
        if withServiceInJson and serviceParam then
        ...
        -- If delivery is "query", we add service as query param instead of JSON
        local url = baseUrl .. "api/reader/get"
        if not withServiceInJson and serviceParam and delivery == "query" then
            url = url .. "?service=" .. serviceParam
        end
        return http_post_retry(url, jsonBody, { headers = headers }, 3)
    end

    -- First attempt: use JSON body (with service if delivery=="json")
    local r = doRequest(delivery == "json")

    -- If that fails or returns raw when we expected translated, retry with query param fallback
    if r.success then
        ...
        -- Check if we expected translated but got raw
        local isTranslatedExpected = (mode == "web" or mode == "webplus")
        if isTranslatedExpected and serviceParam then
            -- Extract body from response to check if it's raw
            local data = json and (json.data and json.data.data or json.data) or nil
            if data and data.body then
                local rawBody = type(data.body) == "table" and json_stringify(data.body) or tostring(data.body)
                -- If rawBody is not encrypted and looks Chinese, maybe we got raw by mistake
                if rawBody and not string_starts_with(rawBody, "arr:") and isLikelyRaw(rawBody) then
                    log_info("wtrlab: response appears raw, retrying with service as query param")
    ...
    -- ── v2 глоссарий книги ────────────────────────────────────────────────────
    -- Load glossary only for ai and webplus (they use term replacement)
    ...
    -- ── Глоссарий главы ───────────────────────────────────────────────────────
    -- Build glossary for ai and webplus only
    local glossary = {}
    if mode == "ai" or mode == "webplus" then
    ...
    if data.patch and (mode == "ai" or mode == "webplus") then
        ...
        log_info("wtrlab: no patches or not applicable for mode " .. mode)
    end
...
--[[
-- To test the different modes, run these in a Lua environment with the plugin loaded:
-- set_preference("wtrlab_mode", "raw")      -- then call getChapterText for any chapter URL
-- set_preference("wtrlab_mode", "web")
-- set_preference("wtrlab_mode", "webplus")
-- set_preference("wtrlab_mode", "ai")
-- Example:
-- local url = "https://wtr-lab.com/en/novel/10563/being-a-talent-in-the-chusheng-demon-sect/chapter-1501?service=web"
-- local html = http_get(url).body
-- local text = getChapterText(html, url)
-- print(text)
--]]