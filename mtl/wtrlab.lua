...
-- Extract chapter text from HTML (for web/webplus/raw modes)
local function parseChapterFromHtml(html)
    -- Remove unwanted elements
    local cleaned = html_remove(html, "script", "style", ".ads", ".nav-links", ".header", ".footer")
    
    -- Try to find the chapter content container
    -- The content is often inside a div with class "chapter-wrap" or "reader-container"
    local el = html_select_first(cleaned, ".chapter-wrap, .reader-container, .chapter-content")
    if not el then
        -- Fallback: try to find the main content area
        el = html_select_first(cleaned, "main, article, .content")
    end
    
    if not el then
        -- If no container found, try to extract text from the body
        -- but skip navigation, headers, footers
        local body = html_select_first(cleaned, "body")
        if body then
            return string_trim(body.text)
        end
        return ""
    end
...
function getChapterText(html, chapterUrl)
    ...
    -- For web and webplus modes, parse the content directly from the HTML
    -- since the server already returns translated text when ?service= is present
    if mode == "web" or mode == "webplus" then
        log_info("wtrlab: parsing chapter from HTML for mode=" .. mode)
        local text = parseChapterFromHtml(html)
        if text and text ~= "" then
            -- For webplus, apply glossary and patches if available
            if mode == "webplus" then
                -- Try to get glossary data from the HTML if embedded
                -- For now, we'll just return the parsed text
                -- Glossary enhancement for webplus would require additional API calls
                -- which might not be available in the HTML
                log_info("wtrlab: webplus mode - applying glossary would require additional API calls")
            end
            return text
        else
    ...
    -- For ai and raw modes, or if HTML parsing failed, use the API
    ...
    -- Map mode to translate and language parameters
    ...
    if applyGlossary then
        ...
        log_info("wtrlab: glossary not needed for mode " .. mode)
    end

    -- ── Build glossary mapping for chapter (only for AI) ──────────────────
    local glossary = {}
    if applyGlossary then
    ...
    -- ── Patches (only for AI) ──────────────────────────────────────────────
    local patches = {}
    if applyGlossary and data.patch then
