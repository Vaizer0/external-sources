...
function getChapterText(html, chapterUrl)
    ...
    if mode == "raw" then
        translateParam = "web"
        languageParam = "zh"      -- force original Chinese
    elseif mode == "web" then
        translateParam = "web"
        languageParam = "en"      -- Google translation to English
    elseif mode == "webplus" then
        translateParam = "webplus"
        languageParam = "en"      -- Glossary Enhanced + Google translation
    elseif mode == "ai" then
        translateParam = "ai"
        languageParam = "none"    -- AI translation (no specific language)
    else
        translateParam = "ai"
        languageParam = "none"
    end
    ...
    -- ── v2 глоссарий книги ────────────────────────────────────────────────────
    -- Only needed for AI and Web+ (which use glossary)
    ...
    -- ── Глоссарий главы ───────────────────────────────────────────────────────
    -- Apply glossary only for AI and Web+ modes
    local glossary = {}
    if mode == "ai" or mode == "webplus" then
