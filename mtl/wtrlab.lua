...
function getChapterList(bookUrl)
    ...
    for i = 1, #chaptersData do
        ...
        -- Append service parameter for web/webplus modes (just for display; API will use translate+service)
...
-- Extract chapter text from HTML (fallback for raw mode)
...
function getChapterText(html, chapterUrl)
    ...
    elseif mode == "web" then
        translateParam = "web"
        languageParam = "en"      -- Google translation
        serviceParam = "web"      -- signal to use web translation
        applyGlossary = false
    elseif mode == "webplus" then
        translateParam = "webplus"
        languageParam = "en"      -- Google translation + glossary
        serviceParam = "webplus"  -- signal for enhanced translation
        applyGlossary = true
    elseif mode == "ai" then
        translateParam = "ai"
        languageParam = "none"
    ...
    if serviceParam then
        bodyObj.service = serviceParam
    end

    local requestBody = json_stringify(bodyObj)
    ...
    if not r.success then
        -- If API fails and mode is raw, fallback to parsing HTML
        if mode == "raw" then
            log_info("wtrlab: API failed for raw, falling back to HTML parsing")
    ...
    if json.success == false then
        ...
        -- If it's a login error and mode is web/webplus, maybe we can fallback to HTML?
        -- But for web/webplus the HTML is likely empty, so we just error
    ...
    -- ── Load glossary only if needed (ai or webplus) ──────────────────────
    ...
    -- ── Build glossary mapping for chapter (only if applyGlossary) ──────
    local glossary = {}
    if applyGlossary then
