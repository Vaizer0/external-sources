...
function getChapterText(html, chapterUrl)
    ...
    -- Map mode to translate parameter
    ...
    -- Load glossary for AI and Web+ modes
    ...
    -- ── Глоссарий главы ───────────────────────────────────────────────────────
    -- In raw mode the text is Chinese, there are no ※idx⛬ markers — glossary is not needed.
    -- In ai and webplus modes: we look up by Chinese original, found → v2 variant, else raw from AI.
    local glossary = {}
    if mode == "ai" or mode == "webplus" then
        if data.glossary_data and data.glossary_data.terms then
            ...
            for i = 1, #terms do
                ...
                if type(termEntry) == "table" then
                    local idx = i - 1 -- 0-based, matches markers ※idx⛬
