...
function getChapterList(bookUrl)
    ...
    for i = 1, #chaptersData do
        ...
        -- Append service parameter for web/webplus modes (only for display, API uses translate)
...
function getChapterText(html, chapterUrl)
    ...
    local languageParam
    local applyGlossary = false  -- only for webplus

    if mode == "raw" then
        translateParam = "web"      -- raw (Chinese)
        languageParam = "zh"
        applyGlossary = false
    elseif mode == "web" then
        translateParam = "webplus"  -- Google translation
        languageParam = "en"
        applyGlossary = false
    elseif mode == "webplus" then
        translateParam = "webplus"  -- Google translation + client glossary
        languageParam = "en"
        applyGlossary = true
    elseif mode == "ai" then
        translateParam = "ai"
        languageParam = "none"
        applyGlossary = true       -- AI also uses glossary
    else
        translateParam = "ai"
        languageParam = "none"
        applyGlossary = false
    ...
    -- Build API request (no service parameter – translate decides)
